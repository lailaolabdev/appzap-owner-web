...
export default function Categorylist() {
    ...
    const _updateCategory = async (values) => {
        try {
            ...
            const resData = await axios.put(END_POINT_SEVER + `/v3/category/update`, {
                id: dataUpdate?._id,
                data: {
                    name: values?.name,
                    note: values?.note,
                                ...
                                {Categorys && Categorys.map((data, index) => {
                                    return (
                                        <tr>
                                            ...
                                            <td>{data?.name}</td>
                                            <td>{data?.note}</td>
                                            <td><FontAwesomeIcon icon={faEdit} style={{ color: COLOR_APP }} onClick={() => handleShow2(data)} /><FontAwesomeIcon icon={faTrashAlt} style={{ marginLeft: 20, color: "red" }} onClick={() => handleShow3(data?._id, data?.name)} /></td>
                                        </tr>
                                    )
                                })}
                            </tbody>
