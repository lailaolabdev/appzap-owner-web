...
import { Breadcrumb, Button, Card, Form, Modal, Pagination, Spinner } from 'react-bootstrap'
...
export default function ZoneList() {
    ...
    const _delete = async () => {
        try {
            setIsLoading(true);
            const data = await Axios.delete(`${END_POINT_SEVER}/v3/zone/${dataDelete?._id}`,
            {
                headers: {
                    Authorization: token
                }
    ...
    return (
        <Box sx={{ padding: { md: 20, xs: 10 } }}>
            <Breadcrumb>
                <Breadcrumb.Item onClick={() => navigate(`/settingStore/${params?.id}`)}>{t('setting')}</Breadcrumb.Item>
