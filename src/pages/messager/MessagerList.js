...
import { warningAlert } from "../../helpers/sweetalert";
...
export default function MessagerList() {
  ...
  const _getDataMessagerList = async (storeId) => {
    const resData = await axios({
      method: "get",
      url:
        END_POINT + `/v3/admin/chat-rooms?storeId=` + userData?.data?.storeId,
    });
  ...
  return (
    <div
      style={{ paddingLeft: 40, display: "flex", flexDirection: "row" }}
        ...
        <Table striped bordered hover>
          ...
          <tbody>
            {dataMessagerList?.map((item, index) => (
              <tr
                key={"message-" + index}
                onClick={() => _showMessageDetail(item.code)}
              ...
              >
                ...
                <td>{item?.code}</td>
                <td>{item?.text}</td>
                <td style={{ color: !item?.isRead ? "red" : "green" }}>
                  {!item?.isRead ? (
      ...
      >
            ...
            {dataMessagerDetail?.map((item) => (
              <p
                style={{
                  ...
                  borderRadius: 8,
                  backgroundColor:
                    item?.isFrom === "TABLE" ? "#E3E3E3" : "#E4E4E4",
                  padding: 10,
                }}
              >
                {item?.text}
        ...
        <InputGroup className="mb-12" style={{ padding: 10, marginLeft: 0 }}>
          <Form.Control
            ...
            className="col-10"
            value={dataForSent}
            onChange={(e) => setDataForSent(e?.target?.value)}
          />
          <Button
            onClick={() => _sentMessagerToCustomer()}
            className="col-2"
            style={{
