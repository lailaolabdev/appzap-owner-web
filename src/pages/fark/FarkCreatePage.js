...
export default function FarkCreatePage() {
	...
	const handleClickCreateBillFark = async () => {
		try {
			...
			const menus = menuFarkData.filter((e) => e?.addToCart);
			const menusFormat = menus.map((e) => ({
				menuId: e?._id,
				amount: e?.cartCount,
			}));
			const _body = {
	...
	return (
		<>
			<div
				style={{
					display: "grid",
					gridTemplateColumns: width > 700 ? "1fr 450px" : "1fr",
				...
				{width <= 700 ? (
					...
					>
						...
						<span style={{ marginLeft: "5px" }}>
							({menuFarkData?.filter((e) => e?.addToCart)?.length})
						</span>
					</button>
				) : null}
			</div>
			<Modal centered onHide={() => setCartModal(false)} show={cartModal}>
				<Modal.Header closeButton>{t("create_dps_list")}</Modal.Header>
				<Modal.Body>
					<div
						style={{
							minHeight: "200px",
					...
					>
						...
						>
							...
							{menuFarkData?.filter((e) => e?.addToCart).length > 0 ? (
								<div
									style={{
										width: "100%",
										padding: "20px",
