	...
	const OPTION = {
		...
		xaxis: {
			labels: {
				formatter: (value) =>
					value
						? `${moment(value).format("DD-MM-YYYY")} (${convertWeekDay(moment(value).weekday())})`
						: 0,
				style: {
					fontSize: "10px",
				},
	...
	const getIncomeExpendData = async () => {
		try {
			...
			if (dateStart && dateEnd)
				findby += `&date_gte==${dateStart}&date_lt=${moment(moment(dateEnd)).format("YYYY/MM/DD")}`;
			...
			if (dateStart && dateEnd)
				findIncomeby += `startDate=${moment(moment(dateStart)).format("YYYY-MM-DD")}&endDate=${moment(moment(dateEnd)).format("YYYY-MM-DD")}`;
