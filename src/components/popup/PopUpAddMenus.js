...
export default function PopUpAddMenus({
    ...
    const getCategory = async (id) => {
        try {
            await fetch(
                master_menu_api_dev + `/api/menus/res-category/` + id,{method: "GET"}
            ).then((response) => response.json())
            .then((json) => setMenuRestaurantType(json?.map((item) => {
                    ...
                    <Row>
                        <Col md="12">
                            <div style={{ marginBottom: 10 }}>
                                ...
                                <select className="form-control" onChange={(e) => setMenuWithRestaurant(e.target.value)}>
                        ...
                        <Col md="12">
                            <table className="table table-hover">
                                ...
                                <tbody>
                                    {menuRestaurantType.length > 0 ? menuRestaurantType?.map((item, idx) => {
                                        return (
                                            <tr key={idx}>
                                                <td>
                                                    <Checkbox
