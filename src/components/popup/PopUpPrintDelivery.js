...
import { getDeliveryReport } from "../../services/report";
...
export default function PopUpPrintDelivery({ open, onClose, children }) {
  ...
  // useEffect(() => {
  //   let _objectA = {};
  //   for (let menu of deliveryReport) {
  ...
  // }, [deliveryReport]);
  useEffect(() => {
    getDeliveryReportData(startDate);
  }, [startDate]);
  ...
  const getDeliveryReportData = async (startDate) => {
    try {
  ...
  return (
    <Modal show={open} onHide={onClose} size="md">
      ...
      >
        ...
        >
          <Container>
            <div style={{ fontWeight: "bold", fontSize: 24 }}>Delivery</div>
