...
import { getMenuCancelReport } from "../../services/report";
...
export default function PopUpPrintMenuCancelHistoryComponent({
  open,
  onClose,
  children,
}) {
  ...
  const handleSearchInput = (option) => {
    if (option?.value?.shiftID === "ALL") {
      setShiftId(null);
      getMenuCancelReportData(startDate);
      setShiftDate(null);
    } else {
  ...
  const getMenuCancelReportData = async (startDate) => {
    try {
      const data = await getMenuCancelReport(storeDetail?._id, findByData());

      // console.log("Menu Cancel Report Data:", data);

      setMenuReport(data.data);
    } catch (err) {}
  };
          ...
          <Container>
            ...
            <TableComponent>
              <tr style={{ fontWeight: "bold" }}>
                ...
                <td style={{ textAlign: "center" }}>{t("cancled_by")}</td>
              ...
              {menuReport?.map((e, i) => (
                <tr>
                  ...
                  <td style={{ textAlign: "center" }}>{e?.menuName || "--"}</td>
                  {/* <td style={{ textAlign: "center" }}>
                    {e?.categoryName || 0}
                  </td> */}

                  <td style={{ textAlign: "right" }}>
                    {moneyCurrency(e?.avgCancelAmount || 0)}
                  </td>
                  <td style={{ textAlign: "center" }}>{e?.cancelBy || "--"}</td>
                  {/* <td style={{ textAlign: "right" }}>{e?.cancelCount || 0}</td> */}
                  {/* <td style={{ textAlign: "right" }}>{e?.note || "--"}</td> */}
                </tr>
              ))}
            </TableComponent>
          </Container>
