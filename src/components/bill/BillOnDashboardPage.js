...
export default function BillOnDashboardPage({
  storeDetail,
  orderPayBefore,
  selectedTable,
  dataBill,
  ...
  useEffect(() => {
    _calculateTotal();
    getDataCurrency();
  }, [taxPercent, storeDetail?.serviceChargePer]);

  // function
  const _calculateTotal = () => {
    let _total = 0;
    for (let _data of (dataBill?.orderId || []).filter(
