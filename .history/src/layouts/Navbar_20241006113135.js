...
export default function NavBar() {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState("LA"); // เพิ่ม state นี้สำหรับภาษา
  ...
  const switchLanguage = (language) => {
    ...
    localStorage.setItem("language", language); // เก็บค่าภาษาใน localStorage
    setSelectedLanguage(language); // อัปเดต state ของภาษา
  };

  useEffect(() => {
    const savedLanguage = localStorage.getItem("language"); // ดึงค่าภาษาเดิมจาก localStorage
    if (savedLanguage) {
      i18n.changeLanguage(savedLanguage); // เปลี่ยนภาษาใน i18n
      setSelectedLanguage(savedLanguage); // อัปเดต state ให้ตรงกับค่าที่เก็บไว้
    }
  }, []);
    ...
    <div>
      ...
      >
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          ...
          >
            {/* ใช้ value={selectedLanguage} เพื่อแสดงภาษาปัจจุบัน */}
            <select
