...
export const orderStatus = (status, t) => {
  switch (status) {
    ...
    case "DOING":
      return t("cooking"); // Properly call t() for translation
    case "SERVED":
      return `ເສີບແລ້ວ`; // Direct Lao string
    case "CART":
      return `ກຳລັງຈະສັງ`; // Direct Lao string
    case "FEEDBACK":
      return `ສົ່ງຄືນ`; // Direct Lao string
    default:
      return "ຍົກເລີກ"; // Default Lao string for cancellation
  }
};
...
const MyComponent = () => {
  const { t } = useTranslation(); // Get the `t` function using `useTranslation`

  const statusText = orderStatus("DOING", t); // Pass `t` to `orderStatus`

  return <div>{statusText}</div>; // Render the translated status
};
