...
// Updated function to accept `t` for translation
export const orderStatus = (status) => {
  ...
  switch (status) {
    case "WAITING":
      return "ອໍເດີເຂົ້າ";
    case "DOING":
      return t("cooking"); // Use the passed translation function
    case "SERVED":
      return `ເສີບແລ້ວ`;
    case "CART":
      return `ກຳລັງຈະສັງ`;
...
// No changes needed here, as this doesn't need translation
...
const MyComponent = () => {
  const { t } = useTranslation(); // Get the translation function from the hook

  const statusText = orderStatus("DOING", t); // Pass `t` into `orderStatus`
