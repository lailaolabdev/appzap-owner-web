...
export const orderStatus = (status) => {
  const { t } = useTranslation(); // Access the translation function here
  
  switch (status) {
    case "WAITING":
      return "ອໍເດີເຂົ້າ"; // Direct translation
    case "DOING":
      return t("cooking"); // Translatable key
    case "SERVED":
      return `ເສີບແລ້ວ`; // Direct translation
    case "CART":
      return `ກຳລັງຈະສັງ`; // Direct translation
    case "FEEDBACK":
      return `ສົ່ງຄືນ`; // Direct translation
    default:
      return t("cancelled"); // Translatable key for "cancelled"
  }
};
};
