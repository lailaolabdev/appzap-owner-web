...
export default function PopUpDetaillBillFark({
  ...
  return (
    <Modal show={open} onHide={onClose} size="md">
      ...
      >
        <div>
          <div>code ລະຫັດ: {billFarkData?.code}</div>
          <div>name ຊື່: {billFarkData?.customerName}</div>
          <div>phonenumber ເບີໂທ: {billFarkData?.customerPhone}</div>
          <div>
            status ສະຖານາະ:
          ...
          <div>
            ...
            {moment(billFarkData?.createdAt).format("DD/MM/YYYY")}
          </div>
          <div>
            ວັນໝົດກຳນົດ Expiry date :{" "}
            {moment(billFarkData?.endDate).format("DD/MM/YYYY")}
          </div>
          <div>
            The day came ວັນມາເອົາ:
        ...
        <div>
          <table style={{ width: "100%" }}>
            <tr>
              <th>ຊື່</th>
              <th style={{ textAlign: "center" }}>amount ຈຳນວນ</th>
            </tr>
            {orderFarkData?.map((e) => (
              <tr>
