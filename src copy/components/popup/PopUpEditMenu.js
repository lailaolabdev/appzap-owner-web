...
export default function PopUpEditMenu({ open, onClose, value, onSubmit }) {
  // state
  ...
  return (
    <Modal show={open} onHide={onClose} keyboard={false}>
      <Modal.Header closeButton>
        <Modal.Title>ແກ້ໄຂເມນູອາຫານ</Modal.Title>
      </Modal.Header>
      <Formik
        initialValues={{
          name: value?.name,
            ...
            <Modal.Body>
              ...
              {isOption && !isStockMulti && !isStockOne && (
                ...
                >
                  ...
                  >
                    {values?.menuOptionId?.map((e, i) => (
                      <li key={i}>
                        <span style={{ fontWeight: "bold" }}>{e?.name}</span>{" "}
                        <span style={{ color: "#ccc" }}>
                          ລາຄາ (
                          <span style={{ color: "#000", fontWeight: "bold" }}>
                            {moneyCurrency(e?.price)} ກີບ
                          </span>
                          )-
                        </span>
                        <span
