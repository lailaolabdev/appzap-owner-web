...
function Cart() {
    ...
    const createOrder = async (data, header, isPrinted) => {
        try {
            ...
            axios.post(END_POINT_SEVER + "/v3/public/bill/create", {
                orders: data,
        ...
        <div>
            <div style={{ padding: 10 }}>
                <FontAwesomeIcon
                    ...
                    // onClick={() => history.push(selectedMenu)}
                    onClick={() => history.replace('/menus/' + match?.params?.storeId + "/" + match?.params?.tableId, selectedMenu) }
                />
            </div>
                                ...
                                return (
                                    <tr key={"selectMenu" + index}>
                                        ...
                                        <td><i
                                            ...
                                            className="fa fa-trash" aria-hidden="true" style={{ color: '#FB6E3B', cursor: "pointer" }}></i></td>
                                    </tr>
                                )
                            })
                        }
